  ...
  return {
    ...
    init: function(visid, filename, props, callback) {
      ...
      visgexf.sig.parseJson(filename, function(){
        ...
        if (0 === visgexf.nodelabels.length) {
          visgexf.sig.iterNodes(function(n){
            ...
            */
            visgexf.nodelabels.push(n.id);
            visgexf.nodemap[n.id] = n.id;
      ...
      visgexf.sig.bind('upnodes', function(event){
        ...
        document.location.hash = hnode.id;
      });

      visgexf.sig.bind('downnodes', function(event){
        dragged = false;
      });
      $('#' + visid).on('mousemove', function (event) {
        dragged = true;
      });
      ...
      (function(sigInst){
        ...
        function showNodeInfo(event) {
          ...
          popUp = $(
            ...
            'position': 'absolute',
            'left': node.displayX + Math.max(2, node.displaySize - 1),
          ...
          timeout = setTimeout(function () {
            ...
            $.ajax({
              ...
              $.ajax({
                url: "/user/q?ownerId=" + data.ownerId
              }).done(function (owner) {
                var nickname = owner.nickname || owner.lastname;
                if (!popUp) {
                  return;
                }
                popUp.html('<span><a href="/metaTableRelation?guid=' + node.id + '" target="_blank"><b>' + node.id + '</b></a></span>' +
                  '<br>' +
                  '<span>业务线: <a class="popup-bu" style="cursor:pointer;">' + (node.attr.bu ? node.attr.bu : '') + '</a></span>' +
                  '<br>' +
                  '<span>责任人: <a href="/search?keywords=' + data.ownerId + '" target="_blank">' + (data.ownerId ? nickname : '') + '</a></span>' +
                  '<br>' +
                  '<span>' + (data.comment ? data.comment.slice(0, 50) : '') + '</span>'
                );
                popUp.find(".popup-bu").on("click", function () {
                  var bu = $(this).html();
                  visgexf.highlightBu(bu);
                });
                $('#' + visid).append(popUp);
              });
            });
    ...
    hex2dec: function(hexval) {
      return parseInt('0x' + hexval, 16).toString(10);
    },
    ...
    highlightNode: function(node) {
      ...
      visgexf.sig.iterEdges(function(e){
        ...
        if (n.id == node.id) {
          ...
          visgexf.setOpacity(n, 0.05);
          n.label = null;
        }
      }).draw(2,2,2);
    },

    highlightBu: function(bu) {
      visgexf.reset();

      var redirectFirstNode = function (node) {
        visgexf.sig.position(0,0,1);
        visgexf.sig.goTo(node.displayX, node.displayY, 2);
        visgexf.sig.position(0,0,1);
      };
      ...
      visgexf.sig.iterNodes(function(n){
        ...
        if (e.attr.sourceNode.bu === bu && e.attr.targetNode.bu === bu) {
          e.hidden = 0;
        } else {
          e.hidden = 1;
        }
    ...
    initSearch: function() {
      visgexf.searchInput.typeahead({
        ...
        'updater': visgexf.processPossibleLocalResult
      });
    ...
    processPossibleLocalResult: function (r) {
      if (r) {
        visgexf.redirectHash(r);
      } else {
        changeSource();
      }
    },
